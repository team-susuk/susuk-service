<template>
    ...
    >
        ...
        <input
            v-bind="$attrs"
            v-bind:class="icon ? 'pe-12' : ''"
            x-model="input"
            @input="$emit('update:modelValue', ($event.target as HTMLInputElement).value)"
            class="bg-white rounded-lg placeholder:text-neutral-gray px-4 text-sm min-h-[42px] outline-none py-2 w-full mb-2 border border-light-gray font-medium focus:ring-0 focus:border-light-gray disabled:bg-light-gray"
            :class="class"
        />
